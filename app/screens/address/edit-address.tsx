...
import { updateAddressAPI } from "@/utils/AddressAPI";
...
const EditAddressScreen = () => {
  ...
  const [formData, setFormData] = useState<IAddress>({
    ...
    addressType: parseInt(params.addressType as string, 10) as 0 | 1,
  ...
  const [selectedProvinceName, setSelectedProvinceName] = useState(params.province as string || "");
  const [selectedDistrictName, setSelectedDistrictName] = useState(params.district as string || "");
  ...
  const fetchProvinces = async () => {
    ...
    try {
      ...
      // Preset districts for the selected province
      const selectedProvince = response.data.find(
        (prov: Location) => prov.name === (params.province as string)
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log("Sending update with data:", JSON.stringify(updatedAddress));
      const response = await updateAddressAPI(formData.id, updatedAddress);
